...
CREATE VIEW young AS (
    ...
    FROM users, profiles
    WHERE users.id = profiles.user_id AND profiles.birthday  > '2003-06-10'
);
...
SELECT *, TIMEDIFF((LEAD(created_at) OVER w), (LAG(created_at) OVER w)) AS 'Разница между соседними сообщениями'
FROM messages
